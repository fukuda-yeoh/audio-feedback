...
# --- 軸補正のための並進ベクトル ---
# 人を中心とした座標系での物体の位置: (0.00, 0.00, 0.30)
# 右カメラのローカル座標系での物体の位置: (-0.06, 0.00, 0.27)
# 補正に必要な並進ベクトル = (人を中心とした座標系での位置) - (右カメラのローカル座標系での位置)
# = (0.00 - (-0.06), 0.00 - 0.00, 0.30 - 0.27)
# = (0.06, 0.00, 0.03)
CAMERA_OFFSET_TO_PERSON_CENTRIC = np.array([0.06, 0.00, 0.03], dtype=np.float32)


# --- YOLOモデルのロード ---
# YOLOモデルのパスを適切に設定してください
# 例: 'yolov8n.pt' (公式の事前学習済みモデル) または 'runs/detect/train4/weights/best.pt' (あなたの学習済みモデル)
try:
...
# --- 3D位置の計算と表示 ---
def process_frames_and_detect(pipeline, yolo_model, offset_vector):
    """
    RealSenseカメラからフレームを取得し、YOLOで物体を検出し、
    検出された物体の3D位置を計算し、指定されたオフセットで補正して表示・出力する。
    offset_vector: カメラのローカル座標系から目的の座標系への並進ベクトル
    """
    # 深度とカラーのアラインメント設定
    ...
    try:
        while True:
            ...
            if not color_frame or not depth_frame:
                # print("Warning: Could not get aligned color or depth frame. Skipping frame.")
                continue
                ...
                for box in r.boxes: # 各バウンディングボックスについて
                    ...
                    if 0 <= center_x < RESOLUTION_WIDTH and 0 <= center_y < RESOLUTION_HEIGHT:
                        ...
                        if 0 <= pixel_index < len(vtx):
                            ...
                            # ここで並進補正を適用
                            point_3d_corrected_coord = point_3d_camera_coord + offset_vector
...
def main():
    ...
    try:
        # process_frames_and_detect関数に計算した並進オフセットを渡す
        process_frames_and_detect(pipeline, yolo_model, CAMERA_OFFSET_TO_PERSON_CENTRIC)
    finally:
        # パイプラインを停止
        pipeline.stop()
