...
# --- 3D位置の計算と表示 ---
def process_frames_and_detect(pipeline, yolo_model):
    """
    RealSenseカメラからフレームを取得し、YOLOで物体を検出し、
    検出された物体の3D位置をカメラ自身の座標系で表示・出力する。
    """
    # 深度とカラーのアラインメント設定
    ...
    print("\n--- Starting Object Detection and 3D Position Estimation (Camera Local Frame) ---")
    ...
    try:
        while True:
                            ...
                            detected_objects_info.append(
                                {
                                    ...
                                    "confidence": conf,
                                    "bbox": (x1, y1, x2, y2),
                                    "3d_position_m": point_3d_camera_coord.tolist() # [X, Y, Z] メートル単位
                                }
                            )

                            # 描画されたフレームに3D位置情報を追加 (カメラローカル座標系での値)
                            text_x = x1
                            text_y = y1 - 10 if y1 - 10 > 10 else y1 + 20 # バウンディングボックスの上か下

                            cv.putText(annotated_frame,
                                       f"X:{point_3d_camera_coord[0]:.2f} Y:{point_3d_camera_coord[1]:.2f} Z:{point_3d_camera_coord[2]:.2f}m",
                                       (text_x, text_y),
                                       cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

            # 検出されたオブジェクトの3D位置情報をコンソールに出力 (カメラローカル座標系での値)
            if detected_objects_info:
                print(f"\n--- Detected Objects in Camera Local Coordinate System ---")
            ...
            # ウィンドウに表示
            cv.imshow("RealSense YOLO 3D Position (Camera Local Frame)", annotated_frame)
...
def main():
    ...
    try:
        # process_frames_and_detect関数を呼び出す (変換パラメータなし)
